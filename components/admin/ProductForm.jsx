  ...
  return (
    <form action={action} className="space-y-6 max-w-xl">
      ...
      <div>
        ...
        {previewUrl && (
          ...
          <img
            src={previewUrl}
            alt=""
            className="w-32 h-32 object-cover rounded-md mb-3 border border-gray-200 bg-gray-100"
          />
        )}
        ...
        <input
          ...
          className="w-full bg-white border border-gray-300 rounded-md px-4 py-3 text-black text-sm file:mr-4 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-black file:text-white file:text-xs file:uppercase file:tracking-widest focus-visible:outline-black"
        />
        ...
        {product?.image && (
          <>
            ...
            <p className="text-gray-500 text-xs mt-2">Leave blank to keep the current image.</p>
          </>
        )}

        {processing && (
          <p className="text-black text-sm mt-3 flex items-center gap-2">
      ...
      <button
        type="submit"
        disabled={processing}
        className="bg-black text-white font-medium uppercase tracking-widest text-sm px-8 py-3.5 rounded-md hover:bg-gray-800 transition-colors disabled:opacity-60"
      >
...
function Field({ label, name, defaultValue, type = 'text', step, required }) {
  return (
    <div>
      <label className="block text-xs uppercase tracking-widest text-gray-600 mb-2">{label}</label>
      <input
        type={type}
        name={name}
        step={step}
